...
class SampleServiceCallback(Service):
    ...
    def cb_create(self, tctx, root, service, proplist):
        ...
        vars.add("TENANT", tenant)
        vars.add("DEP-NAME", dep_name)
        vars.add("DEVICE", device)
        vars.add("VNF", 'TheRouter-CSR')
        ...
        with maapi.single_read_trans("", "system", db=ncs.OPERATIONAL) as th:
            ...
            vnf_deployment = op_root.nfvo_rel2__nfvo.nfvo_rel2__vnf_info.nfvo_rel2_esc__esc.vnf_deployment
            if [tenant, dep_name, device] in vnf_deployment:
                vnf_component = vnf_deployment[tenant, dep_name, device].plan.component['TheRouter-CSR']
        ...
        for old_deployment in old_res_list:
            ...
            for vdu in old_deployment.vdu:
                ...
                for old_vm_device in vdu.vm_device:
                    ...
                    #netsim_port to be updated for netsim devices only. Uncomment when testing with netsim.
